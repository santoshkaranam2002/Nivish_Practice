...
import { barchart, bubblechart, particpationdetails, piechart, totaldata, treechartdata } from '../probite.graphql';
...
export class ChartsComponent {
  ...
  azadKashmirDistrictData?: FeatureCollection<Geometry, { ADMIN_UNIT: string, DISTRICT: string, PROVINCE: string }>;
  geomapdata: any[] = []; 
  selectedState: string = ''; 

  showPopup = false;
  districts: any[] = [];
  azadKashmirDistricts : any[]=[];
  ...
  bubblechartdata: any;
  Orgdata: any;
  orgelectionnames: any;
  assemblyelectionyears: any;
  assemblybyelectionyears: any;
...
constructor(public dialog: MatDialog,private ivinservice:IvinService,private apollo:Apollo,private httpClient: HttpClient){}
  ...
  createMap(): void {
    ...
    forkJoin([
      ...
      this.httpClient.get<FeatureCollection<Geometry, { ADMIN_UNIT: string, DISTRICT: string, PROVINCE: string }>>('assets/data/geo/pok-alhasan.geojson'),
      this.httpClient.get<FeatureCollection<Geometry, { ADMIN_UNIT: string, DISTRICT: string, PROVINCE: string }>>('assets/data/geo/pok-districts-alhasan.geojson') 
    ]).subscribe(([stateData, districtData,extraStateData,azadKashmirDistrictData]) => {
      ...
      console.log('Extra state GeoJSON data loaded successfully:', extraStateData);
      console.log('Azad Kashmir Districts GeoJSON data loaded successfully:', azadKashmirDistrictData);
  
      // Store the district data
      this.districtData = districtData;
      this.azadKashmirDistrictData = azadKashmirDistrictData;
      ...
      this.svg.selectAll('path.state')
        ...
        .style('fill', (d: any) => {
          if (this.geomapdata && Array.isArray(this.geomapdata)) {
            const stateData = this.geomapdata.find((item: any) => 
            item.state.toLowerCase() === d.properties.correctedName.toLowerCase());
            if (stateData) {
              console.log('Matched state data:', stateData);
              if (!lightGreenState) {
              ...
              <ul style="margin: 0; padding-left: 20px;">
                ${[...stateData.wins] // Create a copy of the array
                  .sort((a: any, b: any) => b.wins - a.wins) // Sort in descending order by wins
                  .map((w: any) => `<li>${w.party}: ${w.wins}</li>`)
                  .join('')}
              </ul>
            `;
        ...
        // Render the extra state (e.g., Azad Kashmir) on a separate layer
        // this.svg.selectAll('path.extra-state')
        // .data(extraStateData.features)
        // .enter().append('path')
        // .attr('d', path)
        // .attr('class', 'extra-state')
        // .style('fill', 'lightblue')  // Different color to distinguish
        // .style('stroke', 'white')
        // .style('stroke-width', '1px')
        // .on('mouseover', (event: any, d: any) => {
        //   d3.select(event.currentTarget as HTMLElement).style('fill', 'orange');
        //   this.tooltip.transition().duration(200).style('opacity', .9);
        //   this.tooltip.html(d.properties.DISTRICT + ', ' + d.properties.PROVINCE)
        //     .style('left', (event.pageX + 5) + 'px')
        //     .style('top', (event.pageY - 28) + 'px');
        // })
        // .on('mouseout', (event: any, d: any) => {
        ...
        // .on('click', (event: any, d: any) => {
        //   d3.select(event.currentTarget as HTMLElement).style('fill', 'yellow');
        //   console.log('Clicked extra state:', d.properties.DISTRICT, d.properties.PROVINCE);
        //   this.showDistricts(d.properties.PROVINCE, path, projection);
        // });
  ...
  showDistricts(stateName: string, path: d3.GeoPath, projection: d3.GeoProjection): void {
    if (!this.districtData && !this.azadKashmirDistrictData) return;
    ...
    this.azadKashmirDistricts = this.azadKashmirDistrictData?.features?.filter((d: any) => d.properties.PROVINCE === mappedStateName) || [];

    // Combine both districts if necessary (for rendering together)
    this.districts = [...this.districts, ...this.azadKashmirDistricts];
   // Debug: Log the filtered districts
   console.log(`Filtered districts for ${stateName}:`, this.districts);
    this.showPopup = true;
    ...
    svg.selectAll('path')
      ...
      .on('mouseover', (event: any, d: any) => {
        ...
        tooltip.transition()
          .duration(200)
          .style('opacity', .9);
        tooltip.html(d.properties.NAME_2 || d.properties.DISTRICT)
          .style('left', (event.pageX + 5) + 'px')
          .style('top', (event.pageY - 28) + 'px');
      })
      .on('mouseout', (event: any, d: any) => {
...
OrgChartFunctionality() {
  ...
  this.apollo.use('probite').watchQuery({
    ...
    this.Orgdata = data.StateTreeDetails;
    console.log("Orgdata api", this.Orgdata);

    const electionNames = this.Orgdata.map((item: any) => item.ElectionName);

    console.log('ivin election constutency',electionNames);
     
    this.orgelectionnames = Array.from(new Set(electionNames));

    console.log('ivin election constutency',this.orgelectionnames);

    this.assemblyelectionyears = this.Orgdata
      .filter((item: any) => item.ElectionName === 'Assembly Election')
      .map((item: any) => item.ElectionYear);
      
      console.log('assembly election ivin ',this.Orgdata);

    this.assemblybyelectionyears = this.Orgdata
      .filter((item: any) => item.ElectionName === 'Assembly Bye Election')
      .map((item: any) => item.ElectionYear);

       const constituencies = this.Orgdata.map((item: any) => item.ACName);
    console.log('Constituencies (AC Names):', constituencies); 
...
//   const electionNames = this.orgelectionnames;

//   const yearNodes: any[] = [];
//   const acNodes: any[] = [];
//   const connections: any[] = [];

//   const yearToAcMap: { [key: string]: string[] } = {};

//   electionNames.forEach((name: string) => {

//     const yearList = this.Orgdata
//       .filter((item: any) => item.ElectionName === name)
//       .map((item: any) => item.ElectionYear);

//     const acNames = this.Orgdata
//       .filter((item: any) => item.ElectionName === name)
//       .map((item: any) => item.ACName);

//     yearList.forEach((year: any) => {
      // const yearId = year;
//       const yearId = `${name}-${year}`;
//       yearNodes.push({ id: yearId, name: year.toString() });

//       if (!yearToAcMap[yearId]) {
//         yearToAcMap[yearId] = [];
//       }
//       yearToAcMap[yearId].push(...acNames);
//     });

//     yearList.forEach((year: any) => {
      // const yearId = year;
//       const yearId = `${name}-${year}`;
//       connections.push({ from: name, to: yearId });
//     });

//     for (const [yearId, acNamesForYear] of Object.entries(yearToAcMap)) {
//       connections.push(...acNamesForYear.map((acName: string) => ({ from: yearId, to: acName })));
//     }

//     connections.push({ from: shareholder, to: name });
//   });
...
//   Highcharts.chart('OrgChart', chartOptions);
// }
...
OrganizationChart() {
  ...
  // Create nodes for years, AC names, and connections
  const yearNodes: any[] = [];
  const acNodes: any[] = [];
  const connections: any[] = [];

  electionNames.forEach((name: string) => {
    // Find years for each election type
    const yearList = this.Orgdata
      .filter((item: any) => item.ElectionName === name)
      .map((item: any) => item.ElectionYear);

    yearList.forEach((year: any) => {
      const yearId = `${name}-${year}`; // Unique ID for each year
      yearNodes.push({ id: yearId, name: year.toString() });

      // Find AC names for the specific year
      const acNamesForYear = this.Orgdata
        .filter((item: any) => item.ElectionName === name && item.ElectionYear === year)
        .map((item: any) => item.ACName);

      // Connect the year to each AC name for that year
      acNamesForYear.forEach((acName: string) => {
        const acId = `${yearId}-${acName}`; // Unique ID for each AC name
        acNodes.push({ id: acId, name: acName });
        connections.push({ from: yearId, to: acId });
      });

      // Connect election types to years
      connections.push({ from: name, to: yearId });
    });

    // Connect election types to shareholder (state)
    connections.push({ from: shareholder, to: name });
  });
