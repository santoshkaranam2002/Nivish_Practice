...
@Component({
  selector: 'app-publishandcommunication',
  templateUrl: './publishandcommunication.component.html',
  styleUrls: ['./publishandcommunication.component.scss'],
  animations: [
...
export class PublishandcommunicationComponent {
  ...
  ngOnInit() {
    this.url = sessionStorage.getItem('url');
    if (this.url) {
      const matchResult = this.url.match(/publishevent\/(\d+)/);
...
geteventdetails(){
  ...
  //   // console.log("1111111111",this.eventdetaisid);
  // } else {
  //   this.eventdetaisid = localStorage.getItem('eventdetailsid');
  //   // console.log("????/////",this.eventdetaisid);
  // }
  ...
  this.ivinservice.Geteventdetails(eventDetailsId).subscribe((data:any)=>{
    if(data['Status']===200){
      ...
      // console.log('publishandcommunication',this.eventdetailsitems);
